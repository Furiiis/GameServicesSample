...
namespace Windows {
    ...
    LogsView::LogsView(Event::EventBus* eventBus, UIModel::LogMainModel logs)
        : evBus(eventBus), data(std::move(logs)) {
        Event::Listener addLogEventListener;
        addLogEventListener.Listen<Events::AddLogEvent>(
            [this](const Events::AddLogEvent& event) {
                this->AddLogEvent(event);
            });
        evBus->AddListener(std::move(addLogEventListener));

    }

    void LogsView::AddLogEvent(const Events::AddLogEvent& event) {
        data.AddLog(event.log);
    }

    void LogsView::ShowWindow(bool& check) {
        if (!ImGui::Begin("Logs", &check))
        {
            ImGui::End();
            return;
        }
